    ...
    public function index(Request $request){
        ...
        if($memo){            
            $grupos = DB::table('tbl_cursos as tc')->select('convenios.fecha_vigencia','tc.*',DB::raw("'$opt' as option"),'ar.turnado as turnado_solicitud', 
                'ar.comprobante_pago','e.memo_soporte_dependencia as soporte_exo')
               ...
               $grupos = $grupos->groupby('tc.id','ar.turnado', 'ar.comprobante_pago','convenios.fecha_vigencia','e.memo_soporte_dependencia')->get();       
